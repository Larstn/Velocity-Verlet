...
%                 end
            ...
            if cnt > 0 
                ...
                diff_dGdEx = ...
                    sum(sum(sum(relative_n_norm(dGdEx,dGdEx_prev,2))));
                diff_dGdEy = ...
                    sum(sum(sum(relative_n_norm(dGdEy,dGdEy_prev,2))));
                diff_dGdEz = ...
                    sum(sum(sum(relative_n_norm(dGdEz,dGdEz_prev,2))));
                disp('Nt:')
                disp(Nt)
                disp(diff_dGdEx)
                disp(diff_dGdEy)
...
%                 disp(delta_z)
                ...
                if (diff_dGdEx < 0.01) && (diff_dGdEy < 0.01) &&...
                        (diff_dGdEz < 0.01)

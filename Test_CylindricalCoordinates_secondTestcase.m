...
y_p  = 0*(elementary_charge/atomic_mass);
z_p  = 0;
vx_p = 1;
vy_p = 0;
vz_p = 0;
...
E_x = ones(Nx,round(Ny/2),Nz);
E_r = -ones(Nx,round(Ny/2),Nz)*0.7;
E_z = zeros(Nx,round(Ny/2),Nz);
...
hold on
plot(x_p,y_p,'rx')
plot(xv3(1:100,1),xv3(101:200,1),'k')
plot(xv4(1:100,1),xv4(101:200,1),'b')
%axis xy image
colorbar
title('Meas') 
...
for xx = 3:10%1:length(x_grid)
    for yy = 3:10%1:length(y_grid)
        ...
        max(max(Er2-E_x))
        [xv3, accel2] = velocityVerlet3D(ts, xv0(:,1), accelFunc(E_x, Er2, E_z));
        [xv4, accel2] = velocityVerlet3D(ts, xv0(:,2), accelFunc(E_x, Er2, E_z));
...
for xx = 3:10%1:length(x_grid)
    for yy = 3:10%1:length(y_grid)
        ...
        max(max(Ez2-E_z))
        [xv3, accel2] = velocityVerlet3D(ts, xv0(:,1), accelFunc(E_x, E_r, Ez2));
        [xv4, accel2] = velocityVerlet3D(ts, xv0(:,2), accelFunc(E_x, E_r, Ez2));
...
%%
delta = 1e-3;
dGdVxr_meas = 0*dGdV_xr(:,:,1);


for xx = 1:11%1:length(x_grid)
    for yy = 1:6%1:length(y_grid)
          fprintf('%i, %i\n', xx, yy);
            V2 = V;
            V2(xx,yy,1) = V2(xx,yy,1) + delta;
    ...
    n_charges, n_masses);
            ...
            max(xv-xv3)
            max(xv2-xv4)
            dGdVxr_meas(xx,yy) = dGdVxr_meas(xx,yy) + (G2-G)/(delta);

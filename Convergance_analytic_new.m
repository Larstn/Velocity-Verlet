...
for i = 1:length(Nt_vec)
    ...
    xv0 = [0; 0; 0; 0; 0; 0]*(elementary_charge/electron_mass);
    ...
    x_grid = linspace(0, 2e-2, Nx);%*(elementary_charge/electron_mass);
    y_grid = linspace(0, 2e-2, Ny);%*(elementary_charge/electron_mass);
    z_grid = linspace(0, 2e-2, Nz);%*(elementary_charge/electron_mass);
    ...
    E_y         = repmat(meshgrid(y_grid*0),[1 1 2]);%-centeredDiff(V, 2);
